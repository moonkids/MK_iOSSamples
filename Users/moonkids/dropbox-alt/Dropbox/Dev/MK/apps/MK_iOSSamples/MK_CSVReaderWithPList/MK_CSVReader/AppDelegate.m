...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    //プリファレンスへアクセスするために、NSUserDefaultsを取得
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];

    //***************************************
    //アプリの起動時に、NSUserDefaultsの値をチェック
    //***************************************
    
    BOOL isCSVRead = [defaults boolForKey:@"isCSVRead"];
    if (isCSVRead == NO) {
        //csvからデータを読んでいない場合には、csvから読み込み
        NSLog(@"CSV not yet read");
        self.dataArray = [self readCSV:@"23AICHI2"];
        
        //読み込んだcsvデータは、plistに保存しておく
        //（NSArrayやNSDictionaryなどは、writeToFileメソッドでplistとして保存できる）
        [self.dataArray writeToFile:filePath atomically:YES];
        
        //読み込んだことを、プリファレンス（NSUserDefaults）に保存しておく
        [defaults setBool:YES forKey:@"isCSVRead"];
        //すぐに反映するときには、直後にsynchronizeメソッド
...
-(NSArray *)readCSV:(NSString *)csvFileName{
    ...
    NSArray *csvDataArray = [NSMutableArray arrayWithCapacity:0];
